...
class WikiAgg():
   ...
   # Assert YYYY/MM or YYYY/MM/DD format, assuming no hiccups in date values (ex., no 13th month!)
    def assert_time_format(self,time):
        ...
        if 2>=len(split_date)>=3 or len(split_date[0])!=4 or len(split_date[1])!=2:
            raise TypeError('Please use a YYYY/MM format for yearly counts or YYYY/MM/DD for weekly counts')
            return False
    ...
    def most_viewed_articles(self,time):
        ...
        if len(split_date) == 3:
            ...
            date_list = [(start + timedelta(days=i)).strftime('%Y/%m/%d') for i in range(7)]
        
        # Assume monthly
        elif len(split_date) == 2:
            month_start = datetime.strptime(time, '%Y/%m')
            num_days_in_month = monthrange(int(split_date[0]), int(split_date[1]))[1]
            date_list = [(month_start + timedelta(days=i)).strftime('%Y/%m/%d') for i in range(num_days_in_month)]
        else:
            raise TypeError('Please use a YYYY/MM format for yearly counts or YYYY/MM/DD for weekly counts')
            return
        ...
        try:
            for date in date_list:
                url = 'https://wikimedia.org/api/rest_v1/metrics/pageviews/top/en.wikipedia/all-access/%s' % date
                ...
                for article in pageviews_dict["items"][0]["articles"]:
                    if article["article"] in agg_pageviews_dict:
                        agg_pageviews_dict[article["article"]]["views"] += article["views"]
                        if article["views"]>agg_pageviews_dict[article["article"]]["highest_views"][0]:
                            agg_pageviews_dict[article["article"]]["highest_views"] = (article["views"], date)
                    else:
...
class test_WikiAgg(unittest.TestCase):
    def test(self):
        ...
        self.assertIn('List_of_Canadian_federal_general_elections', test.most_viewed_articles('2015/10'))
        self.assertEqual(test.article_view_count('2021/12/31', 'New_Year'), 61224)
        self.assertEqual(test.article_most_viewed_day('2020/02', 'Groundhog_Day'), '2020/02/02')
        self.assertEqual(test.article_most_viewed_day('2019/02', 'Groundhog_Day'), '2019/02/02')
        self.assertRaises(TypeError, test.article_view_count, '201020', 'Gordon_Ramsay')
        self.assertRaises(TypeError, test.article_most_viewed_day, '201020/', 'Gordon_Ramsay')
